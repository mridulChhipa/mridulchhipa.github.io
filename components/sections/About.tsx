        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(min(320px, 100%), 1fr))", gap: 24 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
            ...
            <div style={cardStyle}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                <PositionTimeline
                  org={"Algorithms & Coding Club (ANCC)"}
                  note={"problemsets & workshops"}
                  steps={[
                    { role: "Coordinator", state: "current" },
                    { role: "Executive", state: "past" },
                  ]}
                />
                <PositionLine>
...
type TimelineStep = { role: string; state: "past" | "current" };
...
function PositionTimeline({
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
      <span style={{ fontSize: "var(--text-sm)", color: "var(--text)" }}>
        <span style={{ color: "var(--accent)", fontFamily: "var(--font-mono)" }}>&gt;</span> {org}
      ...
      <div style={{ display: "flex", flexDirection: "column", paddingLeft: 6 }}>
        {steps.map((step, i) => {
          ...
          return (
            <div key={step.role} style={{ display: "grid", gridTemplateColumns: "14px 1fr", columnGap: 10 }}>
              <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
                ...
                {!isLast && (
                  <span
                    aria-hidden="true"
                    style={{ flex: 1, minHeight: 14, width: 2, background: "var(--border)" }}
                  />
                )}
              </div>
              <span
                style={{
                  ...
                  fontSize: "var(--text-xs)",
                  letterSpacing: "var(--tracking-wide)",
                  color: isCurrent ? "var(--text-strong)" : "var(--text-faint)",
                  paddingBottom: isLast ? 0 : 6,
                  textTransform: "uppercase",
                }}
              >
                {step.role}
                {isCurrent ? <span style={{ color: "var(--accent)" }}> · now</span> : null}
              </span>
            </div>
          );
        })}
